...
import { useCampaignDelete, useCampaigns } from "../../../hooks/useCampaigns";

export const CampaignTable: React.FC = () => {
	const { data, isLoading, isError, error } = useCampaigns();
	const { mutate: deleteCampaign, isPending } = useCampaignDelete();
	...
	if (!data || data.length === 0) {
		return (
			<div className="flex justify-center items-center py-8">
				<span className="text-gray-500">No campaigns found.</span>
			</div>
		);
	}
					...
					{data.map((campaign, idx) => (
						<tr key={idx} className="hover:bg-gray-50">
							...
							<td className="px-4 py-3">
								<span
									className={`inline-block px-2 py-1 rounded text-xs font-semibold ${campaign.status === "active"
							...
							<td className="px-4 py-3 text-gray-500">
								{campaign.updated_at.toLocaleDateString('es-ES')}
							</td>
							<td className="px-4 py-3">
								<button
									type="button"
