...
import PreselectedSuspectsTable from "./PreselectedSuspectsTable";
...
const SuspectsTable = () => {
    ...
    const handleNext = () => {
        ...
        <div className="w-full max-w-7xl mx-auto my-10 bg-white rounded-2xl shadow-2xl p-8">
                ...
                <>
                        ...
                        <table className="min-w-full text-sm text-left border-separate border-spacing-y-1">
                            <thead className="sticky top-0 z-10 bg-white shadow">
                                <tr className="text-gray-700">
                                    <th className="px-3 py-3">
                                        <input type="checkbox" checked={checked.size === data?.suspects.length &&
                            ...
                            <tbody>
                                ...
                                {data?.suspects?.map((row, idx) => (
                                    <tr key={row.identification} className={`transition-colors duration-200 border-b last:border-b-0
                            ${isChecked(row.identification) ? "bg-blue-50" : idx % 2 === 0 ? "bg-gray-50" : "bg-white"}
                            hover:bg-blue-100 focus-within:bg-blue-100`}>
                                        <td className="px-3 py-3">
                                            <input type="checkbox" checked={isChecked(row.identification)} onChange={() =>
                                                handleCheck(row.identification)}
                                                className="accent-blue-600 w-5 h-5"
                                                aria-label={`Seleccionar ${row.email}`}
                                            />
                                        </td>
                                        <td className="px-3 py-3 font-medium text-gray-900 max-w-[180px] truncate whitespace-nowrap" title={row.email}>
                                            {row.email}
                                        </td>
                                        ...
                                        <td className="px-3 py-3">
                                            <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${row.status
                                        ...
                                        <td className="px-3 py-3 text-center">
                                            <div className="flex gap-2 justify-center">
                                                <button className="p-2 rounded-full hover:bg-red-100 text-red-600 transition"
                                                    title="Eliminar" onClick={() => { handleDelete(Number(row.id)) }}
                                                >
                                                    <FaTrash />
                                                </button>
                                                <button className="p-2 rounded-full hover:bg-green-100 text-green-600 transition"
                                                    title="Agregar" onClick={() => alert(`Agregar ${row.email}`)}
                                                >
                                                    <IoAdd />
                                                </button>
                                            </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
                        ...
                        ${currentPage === i + 1
                            ...
                            <button onClick={handleNext} disabled={currentPage === pages || pages === 0}
                                className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 disabled:opacity-50 font-semibold transition">
                                Siguiente
                            </button>
                        </div>
