...
export const ConvertLeadToProspect = ({ participant_id, offset, limit }: { participant_id: number, offset: number, limit: number }) => {
    const convertIntoProspect = useConvertIntoLead(participant_id, offset, limit);
    const [isLoading, setIsLoading] = useState(false);
    const handleSendDraft = () => {
        setIsLoading(true);
        convertIntoProspect.mutate(Number(participant_id),
            {
                onSuccess: () => {
                    setIsLoading(false);
                    toast.success("Convertido en Prospecto, ahora se encuentra en la lista de Prospectos");
                },
                onError: (error) => {
                    console.log(error);
                    setIsLoading(false);
                    toast.error("Error al convertir en Prospecto");
                }
            }
        );
    }
    ...
    return (
        <>
            ...
            >
                ...
                <span className="hidden sm:inline">Convertir en Prospecto</span>
            </button>
            )}
        </>
    )    
